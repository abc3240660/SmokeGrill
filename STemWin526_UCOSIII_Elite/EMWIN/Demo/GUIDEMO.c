...
4 shutdown
*/
int g_current_mode=IDLE;//
int g_current_mode_old=IDLE;
// 0-end 1-start
...
int GUIDEMO_MainMenu(int sel);
void GUIDEMO_DayNightSwitch(void);
void GUIDEMO_DayModeSet(void);
int GUIDEMO_ResetInitialUI(void);
void GUIDEMO_UpdateTemp(int *temp_val);
int GUIDEMO_StartupInitialUI(int mode);
int GUIDEMO_BigCircleUI(int is_smoke_ui);
int GUIDEMO_StartupTimerBypassedUI(void);
...
int direct_switch_check(void)
{
		...
		//
		if((g_current_mode==SHUT)&&(g_shutdown_mode_counter==0))
		{
			g_direct_feed=0;
			GUIDEMO_ClearScreen(0,0,480,320);//清除屏幕
			//GUIDEMO_FeedShutdownTmerUI(0);
...
*/
void GUIDEMO_ProgressTimeBar(int time1,int time2, int time3, int just_update_time)
{
	char dispStr[64] = "";
...
//	GUI_FillRect(bottom.x0,bottom.y0,bottom.x1,bottom.y1);
	...
	//更新时间
#ifdef SRAM_MEMDEV
	hMem = GUI_MEMDEV_Create(105, 270, 280, 45);
	GUI_MEMDEV_Select(hMem);
#endif
	...
	sprintf(dispStr,"TIMER // %2d:%.2d:%.2d",time1, time2, time3);
	GUI_DispStringHCenterAt(dispStr, 240, bottom_text_pos.y);
	
#ifdef SRAM_MEMDEV
	GUI_MEMDEV_CopyToLCDAt(hMem, 105, 270);
	GUI_MEMDEV_Delete(hMem);
	GUI_MEMDEV_Select(0);
#endif
}
...
int GUIDEMO_BigCircleUI(int is_smoke_ui)
{
	...
	int pen_size = 0;
	int time_remain = 100;
	int target_val_center = 355;
	int target_val_center_last = 0;
	char dispStr[64] = "";
	int ret=0;
	EC11_STA ec_sta = EC11_IDLE;
	...
	EC11_Clear();
	if (0 == is_smoke_ui) {
		target_val_center = g_temp_val_new.target_val;
	}else if (1 == is_smoke_ui){
		target_val_center = g_temp_val_new.target_smoke;
	}
	...
	do
	{
		...
		if (target_val_center_last != target_val_center) 
		{//更新大圆温度
#ifdef SRAM_MEMDEV
			if (0 == first_enter_flag) {
			...
			{// set smoke
				...
				sprintf(dispStr, "%d", target_val_center);
				GUI_DispStringHCenterAt("   ",      130, 50);
			...
			{//按照target_val_center画圆
				...
				if (1 == is_smoke_ui) {// set smoke
					GUI_DrawArc(130,133,118,118,90-target_val_center*36,90);
				} else {
					if (2 == is_smoke_ui) {
		...
		target_val_center_last = target_val_center;

		if(g_temp_val_new.target_update == 1)
		{//串口更新了target，更新target值
			if(is_smoke_ui == 0)
				target_val_center = g_temp_val_new.target_val;
			else
				target_val_center = g_temp_val_new.target_smoke;
		...
		if (is_smoke_ui != 2) 
		{
			...
			{// dn
				if (is_smoke_ui==1) 
				{
					if (target_val_center > 1) 
					{
						target_val_center -= 1;
						//g_event_val_new.force_flush = 1;
						//temp_val.target_smoke = target_val_center;
			...
			{//clk
				if (1 == is_smoke_ui) 
				{//smoke mode 短按进入startup
					ec_sta = EC11_IDLE;// 忽略该消息
					//g_smoke_val_percent = target_val_center;
					target_smoke_set = target_val_center;
					if (RUN != g_current_mode) {
						return 0;//start up
					} else {
						return 1;//apply
...
void GUIDEMO_AboutRightUI(char *strGrill_pin, char *strNet, char *strBlueTooth, char *strFW,char *strMODE)
{
	char dispStr[64] = "";
	...
	//sprintf(dispStr, "MODEL // %s", strMODE);
	//GUI_DispStringAt(dispStr, 270, 20+26*5);
	//中部灰色条
	if(g_is_night_mode==0)
...
void GUIDEMO_LeftMenuItems(int sel_new,int sel_last,int update, int ui)
{
	...
	if(g_temp_val_new.temp_unit==0)
		MainMenuTextAll[1][1]="UNITS // °F";
	else
		MainMenuTextAll[1][1]="UNITS // °C";
	if (!update)
	{
		//画6个条
...
*/
int GUIDEMO_ShutdownCompleteUI(void)
{
...
{	
	...
	switch(ui_sel)
	{
		...
		case UI_EXIT_OVER_TEMP:
			...
			if(g_is_night_mode!=0)
				GUI_SetColor(USER_COLOR3_GRAY);
			GUI_DispStringAt("NORMAL OPERATING", 50, 135);
...
int GUIDEMO_SetupSubItemsUI(int index_new, int index_last)
{
	int about_flag = 0;
	int time_remain = 10;
	...
	if (EC11_BUT == ec_sta) {
		...
		} else if (3 == menu_index)
		{
			...
			if(2 == g_current_mode)//如果处于run 模式。取消后直接进入run 界面
...
int GUIDEMO_FeedCompleteUI(void)
{
	int time_remain = 10;
	EC11_STA ec_sta = EC11_IDLE;
	int temp_val[5]={31,32,33,34};
...
int GUIDEMO_FeedShutdownTmerUI(int mode)
{
	...
	if (0 == timer_counter) {//时间到
		if (0 == mode) {
			//GUIDEMO_FeedCompleteUI();
			g_current_mode = IDLE;
			return 1;
		} else {
			g_current_mode=IDLE;//IDLE 模式
...
int GUIDEMO_StartupTimerUI(void)
{
	...
	if (EC11_BUT == ec_sta) {
		if (0 == sel) {
			...
			return 0;//进入bypass confirm界面
		} else {
			g_current_mode=IDLE;//进入IDLE 模式
			//GUIDEMO_MainMenu(0);
			return 1;
		}
	}else
...
int GUIDEMO_ShutdownInitialUI(void)
{
	int time_remain = 10;
	EC11_STA ec_sta = EC11_IDLE;
	int sel=0;
...
int GUIDEMO_StartupTimerBypassedUI(void)
{
	...
	do {
		...
		if ((STARTUP ==g_current_mode )&&(0 == g_startup_mode_counter)) {//startup time 倒计时结束
			break;
		}
	...
	if (EC11_BUT == ec_sta) {
		...
		g_run_mode_counter_sec=0;
		Enter_RUN_MODE_SET();
		g_current_mode=RUN;//bypass 进入run模式
		return 2;
	}
...
int GUIDEMO_FeedInitialUI(void)
{
	int time_remain = 10;
	EC11_STA ec_sta = EC11_IDLE;
	int sel=0;
	...
	if (EC11_BUT == ec_sta) {
		if (0 == sel) {//确认
			//GUIDEMO_FeedShutdownTmerUI(0);
			g_current_mode_old=g_current_mode;
			g_current_mode=FEED;//进入feed 模式
...
int GUIDEMO_StartupInitialUI(int mode)
{
	...
	GUIDEMO_ClearScreen(0,0,480,320);//清除屏幕
	if (0 == mode) {
		GUIDEMO_LeftConfirmCancelUI(sel, target_temp_set, target_smoke_set, 0, UI_CONFITRM_CANCEL_STARTUP);
		GUIDEMO_LeftConfirmCancelUI(sel, target_temp_set, target_smoke_set, 1, UI_CONFITRM_CANCEL_STARTUP);
	} else {
		GUIDEMO_LeftConfirmCancelUI(sel, target_temp_set, target_smoke_set, 0, UI_CONFITRM_CANCEL_APLPLY);//画左侧框
		GUIDEMO_LeftConfirmCancelUI(sel, target_temp_set, target_smoke_set, 1, UI_CONFITRM_CANCEL_APLPLY);//画左侧选中项
	}
	...
	if (EC11_BUT == ec_sta) {
		if (0 == sel) {//confirm
			...
			if (g_set_temp != g_temp_val_new.target_val) {
				I_Err_Sum = 0;
				P_out = 0;
				I_out = 0;
				D_out = 0;
			...
			g_temp_val_new.target_val = target_temp_set;
			g_temp_val_new.target_smoke = target_smoke_set;
			if (0 == mode) {
				//GUIDEMO_StartupTimerUI();
				//进入startup 模式，设置startup 计数
				g_current_mode = STARTUP;
			...
			//temp_temp
			//if(RUN == g_run_mode)//如果处于run 模式。取消后直接进入run 界面
					//GUIDEMO_BigCircleUI(2);
			//	return 2;
			//else
			{
...
int GUIDEMO_MainMenu(int sel)
{
	...
	do{
		...
		if ((0 == time_remain%10)&&(help_flag!=1)) {
			GUIDEMO_UpdateTemp(temp_val);//获取最新温度
			GUIDEMO_RightTempUI(temp_val, 1, big_temp, 1);//更新框内4个温度
			GUIDEMO_RightTempUI(temp_val, 1, big_temp, 2);//更新big温度
		}
...
int GUIDEMO_error(int err)
{
	...
	switch(err)
	{
		...
		case 1:
			...
			g_sensor_error_mode_counter=1200;
			break;
		case 2:
	...
	do{
		...
		if ((0 == time_remain%10)) 
		{
			...
			GUIDEMO_RightTempUI(temp_val1, 1, 0, 2);//
		}
		time_remain--;
		...
		if (EC11_BUT == ec_sta)
		{
			ec_sta = EC11_IDLE;
			//GUIDEMO_BigCircleUI(2);
			g_current_mode = IDLE;//出错后进按键取消错误并进入idle模式
			g_fatal_error=0;//Çå³ý´íÎó
			return 0;
			
		}
		delay_ms(100);
	} while((EC11_BUT != ec_sta)||(g_fatal_error!=0));
}
/*************************** End of file ****************************/
